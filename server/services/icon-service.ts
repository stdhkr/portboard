...
const ICON_CACHE_DIR = path.join(os.tmpdir(), process.env.ICON_CACHE_DIR || ICON.CACHE_DIR);
...
export async function extractAppIcon(appPath: string): Promise<string | null> {
	...
	try {
		...
		let icnsFileName: string = ICON.DEFAULT_NAMES[0]; // Default to first option
		try {
		...
		const iconCandidates: string[] = [icnsFileName, ...ICON.DEFAULT_NAMES];
		...
		await execAsync(
			`sips -s format png "${icnsPath}" --out "${cachedIconPath}" --resampleHeightWidthMax ${ICON.RESIZE_SIZE}`,
		);
