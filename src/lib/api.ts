...
export async function fetchDockerLogs(
	containerId: string,
	lines: number = DOCKER.DEFAULT_LOG_LINES,
	since?: string,
): Promise<DockerLogsResponse> {
	let url = `${API_BASE_URL}/logs/${containerId}?lines=${lines}`;
	if (since) {
...
export async function stopDockerContainer(containerName: string): Promise<void> {
	const response = await fetch(`${API_BASE_URL}/ports/stop-container`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
	...
	if (!response.ok) {
		const result: OpenDirectoryResponse = await response.json();
		throw new Error(result.error || `Failed to stop container: ${response.statusText}`);
	}
...
export async function stopDockerCompose(projectDirectory: string): Promise<void> {
	const response = await fetch(`${API_BASE_URL}/ports/stop-compose`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
	...
	if (!response.ok) {
		const result: OpenDirectoryResponse = await response.json();
		throw new Error(result.error || `Failed to stop compose project: ${response.statusText}`);
	}

	const result: OpenDirectoryResponse = await response.json();
	if (!result.success || result.error) {
		throw new Error(result.error || "Failed to stop compose project");
	}
}

/**
 * Open port URL in default browser
 */
export async function openInBrowser(port: number): Promise<void> {
	const response = await fetch(`${API_BASE_URL}/ports/open-in-browser`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
	...
	if (!response.ok) {
		const result: OpenDirectoryResponse = await response.json();
		throw new Error(result.error || `Failed to open in browser: ${response.statusText}`);
	}

	const result: OpenDirectoryResponse = await response.json();
	if (!result.success || result.error) {
		throw new Error(result.error || "Failed to open in browser");
	}
}

/**
 * Get network URL for a port
 */
export async function fetchNetworkURL(port: number): Promise<string> {
	const response = await fetch(`${API_BASE_URL}/ports/network-url/${port}`);
	if (!response.ok) {
		throw new Error(`Failed to fetch network URL: ${response.statusText}`);
	}
	...
	if (!result.data) {
		throw new Error("Could not determine network URL");
	}
